...
- 在按键自定义菜单，依次选择按键，选择磁性(M0/M1)，设置键程(单位 0.01mm)，然后校准。设置 Rapid Trigger 参数，通常保持默认即可。设置 Key Code，如果不需要用这个按键则选择 Disabled。
- 然后翻到 RT Result 测试你的触发设置。
- 退回到设置，往后翻，选择保存。
- 翻到第一个选项，退出设置。

对了，它**只能**用旋钮修改配置，**不支持**通过 USB 修改配置。它的固件是**专有**的，没有开源的打算，没有开放接口的打算，见 [issue](https://github.com/RQNG/Micrometer-M07/issues/1)。作者说这款硬件其实比较拉，见[下文](#性能指标)。
